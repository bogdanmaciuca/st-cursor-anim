...
draw(void)
{
    ...
    if (term.c.x != term.ocx || term.c.y != term.ocy) {
        clock_gettime(CLOCK_MONOTONIC, &term.lcctime);
        lastcx = term.ocx;
        lastcy = term.ocy;
    }

    /* get elapsed time from last cursor position change */
    struct timespec now;
    clock_gettime(CLOCK_MONOTONIC, &now);
    int elapsed = TIMEDIFF(now, term.lcctime);
